...
function createRecord($fname, $lname, $email, $phone) {
    ...
    $sql = "INSERT INTO Customers (CountryID, FirstName, LastName, Email, PhoneNumber) VALUES (6, '$fname', '$lname', '$email', '$phone')";
    // $stmt = $conn->prepare($sql);
    // $stmt->bind_param("s", $email);
    // $stmt->execute();
    $conn->query($sql);
